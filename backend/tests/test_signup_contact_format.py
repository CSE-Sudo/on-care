...
@pytest.mark.parametrize(
    ...
    [
        ...
        "e2e-640-a-1789539803863827@oncare.test",
    ],
)
...
def test_clean_email_accepts_reserved_test_domains():
    """`.test` 는 RFC 2606 이 시험용으로 비워 둔 최상위 도메인이다.

    `email-validator` 는 이것을 막는다. 앱(`AppInputRules.email`)은 통과시키므로
    서버만 막으면 기준이 갈라지고, 실 API E2E 가 쓰는 계정이 가입에서 떨어진다.
    """
    for email in ("member@oncare.test", "trainer@on-care.invalid"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "member@oncare..com",
        "member@-oncare.com",
        f"{'a' * 250}@oncare.com",  # `users.email` 컬럼(255)을 넘는다
    ],
)
...
def test_register_accepts_reserved_test_domain(client, _cleanup):
    """실 API E2E 계정(`@oncare.test`)이 가입에서 막히지 않는다.

    `email-validator` 로 보면 여기서 422 가 난다 — 앱은 통과시키는 주소라
    서버만 막으면 기준이 갈라진다.
    """
    email = f"{EMAIL_PREFIX}{uuid4().hex[:10]}@oncare.test"
    r = client.post("/v1/auth/register", json=_payload(email=email))
    assert r.status_code == 201, r.text
