...
- 한국건강증진개발원 `보건소 모바일 헬스케어 운동`(공공데이터포털
  https://www.data.go.kr/data/15068730/fileData.do, 운동명 + METS 376건). 국내에서
  실제로 쓰이는 운동 이름과 계수라 이름 매칭이 가장 잘 붙는다. 이용허락범위는
  **KOGL 제4유형(출처표시·상업적 이용금지·변경금지)** 이다 — 막는 것은 상업적
  이용과 변경이지 재배포가 아니므로, 받은 **그대로**
  `backend/app/data/exercise_met_public.csv` 에 두고 전건을 적재한다(#1651). 유형
  매핑·이름 정규화는 파일이 아니라 적재 시점의 코드(`app.db.init_db`)가 붙인다.
- Compendium of Physical Activities (pacompendium.com). 국제 표준 계수로, 위
  데이터에 없는 종목을 메운다.

아래 목록은 두 자료를 **참고해 손으로 추린 대표값**이다(`source=curated`). 식단
시드(43종)와 같은 성격이고, 공공데이터 전건보다 **먼저** 적재된다 — 회원이 실제로
적는 말(별칭 포함)로 손질한 것이라 원본의 긴 항목명보다 이름 매칭이 잘 붙는다.
계수를 임의로 바꾸지 않는다: 같은 종목이 자료마다 다르면 국내 자료를 따른다.
무엇이 몇 건 적재되고 이름이 얼마나 붙는지는
`python -m scripts.check_exercise_catalog` 로 확인한다.
