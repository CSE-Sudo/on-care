...
**이메일 규칙은 `AppInputRules.email` 과 글자 그대로 같은 것을 본다.** 처음에는
`email-validator` 를 썼는데, 그쪽은 RFC 2606 이 시험용으로 비워 둔 최상위 도메인
(`.test`·`.invalid`·`localhost`)을 막는다. 앱은 통과시키고 서버만 막으니 실 API
E2E 가 쓰는 `@oncare.test` 계정이 가입에서 떨어졌다. 앱이 보여 준 기준과 서버가
받는 기준이 다르면 "화면에서는 괜찮다는데 가입이 안 된다" 가 된다.

**이메일은 형식만 보고 값은 바꾸지 않는다.** 소문자로 고쳐 저장하면 로그인 조회
...
#: 저장 가능한 이메일 길이. `users.email` 컬럼(`String(255)`)에 맞춘다 — 넘치면
...
#: 정도로만 엄격하다. 한쪽만 고치면 두 기준이 갈라지니 함께 고쳐야 한다.
_EMAIL = re.compile(
