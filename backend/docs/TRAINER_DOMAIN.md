...
| POST | `/trainer/clients/{member_id}/program` | 프로그램 배정 — 세션당 루틴 한 건 (#709). `delivery_kind`·`trainer_message`·`start_date`·`repeat_days` 로 프로그램 만들기의 `개인운동만` 전송을 받는다. `repeat_days`(기본 1)만큼 시작일부터 날마다 한 건씩 만든다 — `개인운동만` 은 7 을 보내 보낸 날부터 한 주 동안 회원 앱에 매일 뜨게 한다 (#2223) |
