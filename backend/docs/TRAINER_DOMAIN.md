...
트레이너 예약 시간에는 비는 칸이 생긴다. 트레이너가 `포인트 체험 허용` 을 켜고 연 빈
시간에만, **담당 트레이너가 없는 회원**이 500P 로 20분 자세 점검 체험을 예약한다.

- **종류로 표현한다.** 슬롯 `session_type` 에 `체험` 을 더했다(`1:1 PT|상담|체험`).
  회원 예약이 만드는 일정이 종류를 그대로 물려받으므로, 체험은 따로 잇지 않아도 트레이너
  스케줄·완료·취소·노쇼 흐름에 그대로 들어간다. 불리언 플래그로 두면 "상담 + 체험" 같은 뜻
  없는 조합이 생긴다. 체험 슬롯은 보낸 시간과 상관없이 20분이다.
- **체험 기록을 예약 행과 따로 둔다**(`points_trials`). 회원이 취소하면 예약 행은 지워지는데
  (`_release`), "이 트레이너 체험을 이미 했나" 와 포인트의 결말은 남아야 한다. 상태는
  `booked|completed|no_show|refunded|forfeited`, 결말을 만든 주체는 `settled_by`.
- **담당이 있는 회원은 제외한다** — 다른 트레이너가 기존 회원을 데려가는 갈등을 막는다.
  회원용 슬롯 목록에서 체험 자리를 빼고, 예약도 409 다. 담당 여부는 어느 트레이너든 활성
  링크 하나로 판정한다. 일반 슬롯의 "담당 트레이너 자리만" 규칙은 그대로다.
- **트레이너별 1인 1회.** 반환된 체험은 세지 않는다 — 포인트를 돌려받은 예약은 쓴 적이 없는
  것과 같다. 진행 중·완료·노쇼·늦은 취소는 1회로 센다. `(member_id, trainer_id) WHERE status <>
  'refunded'` partial unique index 가 마지막 방어선이다.
- **포인트는 예약과 같은 트랜잭션에서 쓴다.** 잔액 행을 잠근 채 담당·1회·잔액을 보고
  `spend`(source `points_trial`)를 남긴다. 예약이 실패하면 사용도 롤백된다. 잠금 순서는
  슬롯 → 잔액이다(취소 경로도 같다).

| 결말 | 경로 | 포인트 | 체험 상태 |
|---|---|---|---|
| 트레이너 취소 | `POST /trainer/schedule/{id}/cancel`(주체 값과 무관) | 반환 | `refunded` |
| 회원 취소, 시작 24시간 전까지 | `DELETE /reservations/{id}` | 반환 | `refunded` |
| 회원 취소, 24시간 이내 | 같은 경로 | 소멸 | `forfeited` |
| 노쇼 | `POST /trainer/schedule/{id}/no-show` | 소멸 | `no_show` |
| 완료 | `POST /trainer/schedule/{id}/complete` | — | `completed` |
| 트레이너 탈퇴 | 계정 삭제 | 반환 | (기록은 CASCADE) |

회원의 24시간 판정은 서버가 내린다(`MyReservationOut.points_refundable`) — 앱은 이 값으로
취소 확인창의 반환·소멸 문구를 고른다.
