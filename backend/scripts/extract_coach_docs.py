...
SECTIONS: tuple[tuple[str, str, str, str], ...] = (
    ...
    # 무기질 중 나트륨만 예외다: 앱이 식단을 나트륨·당류로 평가하는데(대시보드
    # 주간 점수, 식단 코칭 한 줄) 그 상한의 근거가 DASH 뿐이었다. 만성질환위험
    # 감소섭취량(CDRR)이 같은 수치의 국내 기준이라 근거를 여기로 옮긴다.
    ...
    ("kdri_sodium.txt", "KDRI_2025_국문요약본.pdf", r"^3-1-3 나트륨/염소 나트륨과", r"참고문헌 [A-Z]"),
)
