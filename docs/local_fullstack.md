...
데모 트레이너(윤재희 제외)에게는 상담·예약에서 고를 `1:1 PT` 자리가 둘씩 깔립니다(#2067) —
내일 13:00, 모레 19:30. 기동할 때 깔고, 자리 목록을 읽을 때 고를 자리가 다 떨어졌으면 같은
규칙으로 다시 깔아 오래 켜 둔 서버에서도 비지 않습니다. 윤재희는 자리 없는 상태(헬스장 전화
안내)를 보여 주려고 비워 둡니다. 자세한 규칙은 `backend/API_CONTRACT.md` 상담 요청 절에 있습니다.
...
| 순서 | 단계 | 앱 | 확인하는 것 |
| --- | --- | --- | --- |
| 1 | `member-request` | 회원 앱 | 트레이너가 연 자리를 새 회원 둘이 UI 폼으로 골라 신청 → 서버에 입력값·고른 자리 그대로 |
| 2 | `trainer-accept` | 트레이너 웹 | 인박스에서 내용 확인 → 승인, 고른 자리에 `1:1 PT` 일정 생성 |
...
| 8 | 정리 | — | 상담 일정 삭제 · 이번 실행이 연 자리 닫기 · 계정 둘 삭제 |
...
상담 시각은 트레이너가 열어 둔 `1:1 PT` 자리에서만 고르므로(#1873), `member-request` 가
`trainer-demo` 계정으로 자리 둘(이틀 뒤·사흘 뒤 21:00)을 먼저 엽니다. 시드 자리(#2067)가
함께 보여도 칩을 자리 id 로 짚어 이번 실행이 연 자리를 고릅니다. 정리 단계가 이 자리도
닫습니다 — 두면 다음 실행과 예약 스위트가 보는 자리 목록에 쌓입니다.
