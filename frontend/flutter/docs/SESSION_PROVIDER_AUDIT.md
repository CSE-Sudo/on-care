...
| `myWeeklyReportsProvider` | Auto-dispose, but an open benefits page survives a demo-to-login transition and would keep the previous account's purchased weekly reports (#2022) | Invalidate the leaf |
| `profilePetProvider` | Non-auto-dispose; the MY profile card keeps the pet worn next to the name (#2021) | Invalidate the leaf |
| `demoProfilePetBookProvider` (not reset) | Demo-only profile pet book shared by the local API's exchange and `GET /me/profile-pet` through the coupon book; exchanges spend from `demoPointsLedgerProvider` (#2021) | Kept for the app lifetime together with the point ledger it spends from, for the same reason as the coupon book. The demo has a single member account |
...
| `myStreakShieldsProvider` | Auto-dispose, but an open benefits page survives a demo-to-login transition and would keep the previous account's held shields and protected days (#1788). `streakShieldRepositoryProvider` follows `exerciseRepositoryProvider`, which is already recreated | Invalidate the leaf |
| `demoStreakShieldBookProvider` (not reset) | Demo-only streak shield book shared by the local API, the coupon book, and the mock exercise repository; exchanges spend from `demoPointsLedgerProvider` (#1788) | Kept for the app lifetime together with the point ledger it spends from, for the same reason as the coupon book. The demo has a single member account |
